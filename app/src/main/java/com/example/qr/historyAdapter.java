...
public class historyAdapter extends RecyclerView.Adapter<historyAdapter.myviewholder>{
    ...
    public historyAdapter(ArrayList<Result> dataholder,OnNoteListener onNoteListener) {
    ...
    public void onBindViewHolder(@NonNull myviewholder holder, int position) {
        ...
        boolean ans=false;
        if(check_URL(history.get(position).getText())) {
    ...
    class myviewholder extends RecyclerView.ViewHolder implements View.OnClickListener
    {
        ...
        public myviewholder(@NonNull View itemView,OnNoteListener onNoteListener) {
            ...
            imgButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    Result temp=history.remove(getAdapterPosition());
                    //like.remove(getAdapterPosition());
                    for(Result k1:like)
                    {
                        if(k1.getText().equals(temp.getText())&&k1.getTimestamp()==temp.getTimestamp())
                        {
                            like.remove(like.indexOf(k1));
                            break;
                        }
                    }
                    saveData();
        ...
        @Override
        public void onClick(View view) {
            ScannerFragment.scan=history.get(getAdapterPosition());
